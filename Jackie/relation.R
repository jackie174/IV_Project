...
carpark_result$parking_percentage <- log1p((carpark_result$total_parking_spaces / total_parking) * 100)
# 计算最大的停车百分比
...
employment_result$jobs_percentage <- log1p((employment_result$total_jobs / total_jobs) * 100)
...
transport_result <- transport_data_2020 %>%
  group_by(`Clue.Small.Area`) %>%
  mutate(transport_count = sum(!is.na(Transport))) %>%
  ungroup()
...
server <- function(input, output, session) {
  output$carpark_fig1 <- renderPlotly({
    carpark_fig1 <- plot_ly(
      type = 'scatterpolar',
      r = ~carpark_result$parking_percentage,
      theta = ~carpark_result$Clue.Small.Area,
      fill = 'toself',
      mode = 'markers'
    ) %>%
      layout(
  ...
  output$employment_fig2 <- renderPlotly({
    employment_fig2 <- plot_ly(
      type = 'scatterpolar',
      r = ~employment_result$jobs_percentage,
      theta = ~employment_result$Clue.Small.Area,
      fill = 'toself',
      mode = 'markers'
    ) %>%
      layout (
  ...
  output$transport_fig3 <- renderPlotly({
    transport_fig3 <- plot_ly(
      type = 'scatterpolar',
      r = ~transport_result$transport_count,
      theta = ~transport_result$Clue.Small.Area,
      fill = 'toself',
      mode = 'markers'
    ) %>% layout(
  ...
  output$bus_stop_fig4 <- renderPlotly({
    bus_stop_fig4 <- ggplot(bus_stop_data, aes(x = name, y = val)) +
  ...
  output$liquor_fig5 <- renderPlotly({
    liquor_fig5 <- plot_ly(
      liquor_filtered_data, x = ~liquor_filtered_data$V9, y = ~log(liquor_filtered_data$TOTAL_LICES), 
      type = 'bar', text = ~as.character(liquor_filtered_data$TOTAL_LICES),
      textposition = 'auto',
  ...
  output$population_fig71 <- renderPlot({
    ...
    population_fig71_data_final$individual <- as.factor(population_fig71_data_final$individual)
    
    empty_bar <- 2
    to_add <- data.frame( matrix(NA, empty_bar*nlevels(population_fig71_data_final$individual), ncol(population_fig71_data_final)) )
    colnames(to_add) <- colnames(population_fig71_data_final)
    to_add$individual <- rep(levels(population_fig71_data_final$individual), each=empty_bar)
    
    population_fig71_data_final <- rbind(population_fig71_data_final, to_add)
    population_fig71_data_final <- population_fig71_data_final %>% arrange(individual)
    population_fig71_data_final$id <- seq(1, nrow(population_fig71_data_final))
    
    label_data <- population_fig71_data_final
    number_of_bar <- nrow(label_data)
    angle <- 90 - 360 * (label_data$id-0.5) /number_of_bar     
    label_data$hjust <- ifelse( angle < -90, 1, 0)
    label_data$angle <- ifelse(angle < -90, angle+180, angle)
    
    fig7 <- ggplot(population_fig71_data_final, aes(x=as.factor(id), y=value)) +      
  ...
  output$population_fig72 <- renderPlot({
    ...
    fig7 <- ggplot(population_fig71_data_final, aes(x=as.factor(id), y=value)) +      
      ...
      geom_text(data=label_data, aes(x=id, y=value+10, label=individual, hjust=hjust), color="black", fontface="bold",alpha=0.6, size=4, angle= label_data$angle, inherit.aes = FALSE ) 
